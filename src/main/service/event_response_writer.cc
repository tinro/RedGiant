...
void EventResponseWriter::add_body(const char* body, int size) {
  if (body && size > 0) {
    evbuffer_add(ev_buf_.get(), body, size);
  }
}

void EventResponseWriter::send_done(int status_code, const char* status_msg) {
  evhttp_send_reply(ev_req_, status_code, status_msg, ev_buf_.get());
}

void EventResponseWriter::send_error(int status_code, const char* status_msg) {
